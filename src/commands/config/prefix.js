...
class Prefix extends Command {
  ...
  async run(msg, args) {
    ...
    if(prefix === msg.guild.settings.prefix) throw "Friend! That is already the current prefix.";
  ...
  async reset(msg) {
    if(msg.guild.settings.prefix === "WaveForm") return msg.send("The prefix is already the default.");
    await msg.guild.update({ prefix: "WaveForm" });
    return msg.send("Reset the prefix for this server to `WaveForm`");
  }
}
